...
void AHelloARManager::BeginPlay()
{
	...
	//UKismetSystemLibrary::PrintString(this, "RUNNING ON ANDROID", true, true, FLinearColor(0, 0.66, 1, 1), 2);
...
void AHelloARManager::UpdatePlaneActors()
{
	...
	for (auto& It : Geometries)
	{
		...
		else
		{
			...
			switch (It->GetTrackingState())
			{
				//If tracking, continue spawning planes unless rendering has been toggled off
			case EARTrackingState::Tracking:
				//if (!It->GetSubsumedBy()->IsValidLowLevel() && PlaneIndex < MaxPlanes)
				if (!It->GetSubsumedBy()->IsValidLowLevel() && !StopPlanesRendering)
				{
...
void AHelloARManager::SetStopPlanesRendering(bool Stop)
{
	StopPlanesRendering = Stop;
	...
	for (auto& It : Geometries)
	{
		...
		if (PlaneActors.Contains(It))
		{
			...
			if (CurrentPActor)
			{
				if (StopPlanesRendering)
					CurrentPActor->PlanePolygonMeshComponent->SetVisibility(false);
				else
					CurrentPActor->PlanePolygonMeshComponent->SetVisibility(true);
			}
		}
	}
}
